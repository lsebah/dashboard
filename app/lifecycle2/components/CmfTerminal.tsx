...
const SIT_META: Record<Sit, { label: string; color: string }> = {
  positive: { label: 'En gain', color: '#22c55e' },
  sans_stress: { label: 'Sans stress', color: '#38bdf8' },
  proche: { label: 'Proche barrière', color: '#f59e0b' },
  sous: { label: 'Sous barrière', color: '#ef4444' },
  non_classe: { label: 'Non classé', color: '#64748b' },
}
...
export default function CmfTerminal({ products }: { products: Product[] }) {
  ...
  // Niveaux worst-of courants (batché, une requête).
  useEffect(() => {
    let annule = false
  ...
  // Risque « live » (dépend des niveaux courants).
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* ── Bandeau d'en-tête de section ───────────────────────────────── */}
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-white">Vue d&apos;ensemble du portefeuille</h2>
          <p className="lc2-label normal-case tracking-normal text-slate-500">
        ...
        <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] text-slate-300">
          <span className={`h-1.5 w-1.5 rounded-full ${live ? 'bg-emerald-400' : 'bg-amber-400'} ${live ? 'lc2-pulse' : ''}`} />
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard label="Encours nominal" value={eurCompact(T.nominal)} sub={`${T.nbVivant} produits vivants`} accent="#6366f1" style={{ animationDelay: '0ms' }} spark={<Sparkline data={sparkEncours} color="#6366f1" />} />
        <StatCard label="Valorisation MtM" value={eurCompact(T.valorisation)} delta={T.pnlPct} deltaLabel="vs nominal" accent="#22d3ee" style={{ animationDelay: '40ms' }} />
        <StatCard label="P&L latent" value={`${T.pnlEur >= 0 ? '+' : ''}${eurCompact(T.pnlEur)}`} sub="MtM + coupons encaissés" accent={T.pnlEur >= 0 ? '#34d399' : '#fb7185'} style={{ animationDelay: '80ms' }} />
        <StatCard label="Rdt depuis origine" value={pct(T.pnlPct)} sub="pondéré nominal" accent="#34d399" style={{ animationDelay: '120ms' }} />
        ...
        <StatCard label="Rendement YTD" value={pctAbs(T.carryYtd)} sub="carry couru 2026 · indicatif" accent="#fbbf24" style={{ animationDelay: '200ms' }} />
        <StatCard label="Dispersion P&L" value={`${T.dispersion.toFixed(1)} pt`} sub="écart-type pondéré · indicatif" accent="#a78bfa" style={{ animationDelay: '240ms' }} />
        <StatCard label="Ratio rdt / risque" value={T.ratio.toFixed(2)} sub="rendement / dispersion" accent="#60a5fa" style={{ animationDelay: '280ms' }} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        <Panel title="Évolution de l'encours" sub="nominal actif reconstruit (mensuel)" className="lg:col-span-3">
          <AreaChart points={histo.map((h) => ({ label: h.label, value: h.encours }))} format={eurCompact} color="#22d3ee" />
        </Panel>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        ...
        <Panel title="Indicateurs de risque" className="lg:col-span-2">
          ...
          {/* Situations — barre empilée */}
          <div className="mt-4">
            <div className="lc2-label mb-1.5 normal-case tracking-normal text-slate-500">Situation du livre (live)</div>
            ...
            <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px]">
              {(Object.keys(situations) as Sit[]).map((s) => (
                <span key={s} className="inline-flex items-center gap-1 text-slate-400">
                  <span className="h-2 w-2 rounded-sm" style={{ background: SIT_META[s].color }} />
                  {SIT_META[s].label} <span className="tabular-nums text-slate-300">{situations[s].n}</span>
                </span>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Panel title="Échéancier (maturity ladder)" sub="nominal arrivant à maturité par année">
          <BarList items={echeances.map((e) => ({ label: e.label, value: e.montant, pct: e.pct, sub: `${e.n}`, color: '#60a5fa' }))} format={eurCompact} />
        </Panel>
...
function Stat({ k, v, warn }: { k: string; v: string; warn?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-2 border-b border-white/5 pb-1.5">
      ...
      <span className={`shrink-0 text-right font-medium tabular-nums ${warn ? 'text-amber-400' : 'text-slate-200'}`}>{v}</span>
    </div>
  )
}
...
function PositionsTable({ products, courant }: { products: Product[]; courant: Record<string, number | null> | null }) {
  ...
  const th = (k: SortKey, label: string) => (
    <th
      ...
      className="cursor-pointer select-none whitespace-nowrap px-3 py-2 text-right font-medium hover:text-white"
    >
      {label}
      {sort.k === k ? (sort.dir === -1 ? ' ▼' : ' ▲') : ''}
    </th>
  ...
  return (
    <Panel title="Positions" sub={`${rows.length} lignes vivantes · cliquer un en-tête pour trier`} className="p-0">
      <div className="max-h-[460px] overflow-auto rounded-b-2xl">
        <table className="w-full text-[12px]">
          <thead className="sticky top-0 z-10 bg-[#12121a] text-slate-400">
          ...
          <tbody className="divide-y divide-white/5">
            {rows.map(({ p, nom, prix, pnl, wo, sit, mat }) => (
              <tr key={p.isin} className="hover:bg-white/[0.04]">
                <td className="whitespace-nowrap px-3 py-1.5 font-mono text-slate-300">{p.isin}</td>
                <td className="whitespace-nowrap px-3 py-1.5 text-slate-300">{p.emetteur}</td>
                <td className="max-w-[180px] truncate px-3 py-1.5 text-slate-400" title={p.nom}>{p.productType ?? p.family}</td>
                <td className="whitespace-nowrap px-3 py-1.5 text-right tabular-nums text-slate-200">{eurCompact(nom)}</td>
                <td className="whitespace-nowrap px-3 py-1.5 text-right tabular-nums text-slate-300">{prix.toFixed(1)}</td>
                <td className={`whitespace-nowrap px-3 py-1.5 text-right tabular-nums ${pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{pct(pnl)}</td>
                <td className="whitespace-nowrap px-3 py-1.5 text-right tabular-nums text-slate-300">{typeof wo === 'number' ? `${wo.toFixed(0)} %` : '—'}</td>
                <td className="whitespace-nowrap px-3 py-1.5">
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full" style={{ background: SIT_META[sit].color }} />
                    <span className="text-slate-400">{SIT_META[sit].label}</span>
                  </span>
                </td>
                <td className="whitespace-nowrap px-3 py-1.5 text-right tabular-nums text-slate-400">{mat ? mat.slice(0, 7) : '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
